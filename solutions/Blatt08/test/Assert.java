package Blatt08.test;

import Blatt08.util.MyList;
import Blatt08.util.OpenHashSet;
...
public class Assert {
    ...
    public static <E> void assertEquals(MyList<E> expected, MyList<E> actual) {
       ...
       while (!expected.endpos() || !actual.endpos()){
          if(!expected.elem().equals(actual.elem())){
             System.out.println("FAIL expected " + expected.elem().toString() + " but was " + actual.elem().toString());
          }
          expected.advance();
          actual.advance();
       }
   ...
   public static <E> void assertEquals(OpenHashSet<E> expected, OpenHashSet<E> actual) {
      if(expected.getSize() != actual.getSize()){
         System.out.println("FAIL expected " + expected + " but was " + actual);
      }
      for(int i =0;i<expected.getSize()-1;i++){
         assertEquals(expected.getList(i), actual.getList(i));
      }
   }
}
